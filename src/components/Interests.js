...
const Interests = ({ isLargeScreen, interestsRef }) => {
  ...
  return (
    <section ref={interestsRef} className="bg-lightPrimaryColor text-lightText dark:bg-darkPrimaryColor dark:text-darkText theme-transition py-20">
      <div className="p-8">
            ...
            >
              <div
                className={`cursor-pointer transition-all duration-300
                  ...
                  bg-gradient-to-br
                  ${index % 2 === 0 ? 'from-blue-100 to-purple-100' : 'from-pink-100 to-orange-100'}
                  hover:from-blue-200 hover:to-purple-200
                  ${activeInterest === interest.title && isTouchDevice ? 'scale-110' : ''}`}
                onClick={() => handleInteraction(interest.title)}
                onMouseEnter={() => !isTouchDevice && handleInteraction(interest.title)}
              ...
              <div 
                className={`absolute 
                  ...
                  ${activeInterest === interest.title ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}
                  ${isTouchDevice ? 'touch-card' : ''}`}
                style={{ zIndex: 100 }}
              >
                <p className="text-sm font-medium text-gray-800">{interest.title}</p>
